...
# parse livedata fields
 {
    ...
    export LIVEDATA_SYSTEM_PROTOCOL="$LIVEDATAPROTOCOL_ECOWITT_BINARY"
    export LIVEDATA_SYSTEM_PROTOCOL_LONG="$LIVEDATAPROTOCOL_ECOWITT_BINARY_LONG"
    ...
    while [ "$OD_BUFFER_HEAD" -lt  $parselivedata_max_length ]; do
       ...
       # [ "$DEBUG_PARSE_LIVEDATA" -eq 1 ] && echo >&2 "OD_BUFFER_HEAD $OD_BUFFER_HEAD OD_BUFFER_LENTH $OD_BUFFER_LENGTH"
        ...
        if [ "$ldf" -eq "$LDF_INTEMP" ]; then
            ...
            export LIVEDATA_WINDDIRECTION_COMPASS_VALUE="$VALUE_COMPASS"
