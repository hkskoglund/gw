...
printSensorWH90()
{
        [ -n "$SENSOR_WH90_ID_STATE" ] && printSensorLine "$BACKUPNAME_SENSOR_WH90" 0 "$SENSOR_WH90_ID" "$SENSOR_WH90_BATTERY_INT" "$SENSOR_WH90_SIGNAL"  "$SENSOR_WH90_ID_STATE" "$SENSOR_WH90_BATTERY_STATE" "$SENSOR_WH90_SIGNAL_STATE"
}
...
</html>"   "$l_sysinfo" "$LIVEDATA_SYSTEM_HOST" "$l_sysinfo" "$SENSOR_TEXT_UTF8"
 unset l_sysinfo
