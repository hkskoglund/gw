...
class Packet_Customized extends Packet {
    ...
    constructor(cmd,twoBytePacketLength)
    {
        if (cmd instanceof Buffer) {  // Allow passing of received data and immediate parsing
            super(cmd[2],twoBytePacketLength);
            this.parse(cmd);
        } else 
    ...
    // for write customized 0x2b command
    {
        ...
        let p = new Packet_Customized(GW.Command.WRITE_CUSTOMIZED);
        ...
        p.writeUint8(Number(customized.protocol === GW.Protocol.WUNDERGROUND), Number(customized.enabled === true)).writeCRC();
